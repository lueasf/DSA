...
def find_one_solution_rec(
    depth: int, target_volume: int, containers: List[int], capacities: List[int], log: List[str]) -> Tuple[bool, List[str]]:
    if depth <= 0:
        return False, log
...
def find_one_solution_with_memoization_rec(depth: int,target_volume: int,containers: List[int],capacities: List[int],log: List[str],memory: Set[Tuple[int, ...]],) -> Tuple[bool, List[str]]:
    if depth <= 0:
        return False, log
...
def find_one_solution_breadth_first_search(target_volume: int, capacities: List[int]) -> Tuple[bool, List[str]]:
    ...
    while not solution_found or not states_to_explore:
        containers, log = states_to_explore.popleft() # a voir
